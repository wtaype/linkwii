import{t as e}from"./vendor-BDh6mtVu.js";import{a as t,d as n,f as r,h as i,m as a,r as o,s,t as c}from"./widev-CwEfqxZK.js";import"./wii-hgYUGqM9.js";import{t as l}from"./preload-helper-vcufLG64.js";import{a as u,c as d,f,l as p,m,o as h}from"./firebase-BzxEF9v7.js";import{db as g}from"./firebase-Be0frFSh.js";var _=()=>a(`wiSmile`),v=`epTotalEmpleados`,y=`epTotalEquipos`,b=`epMetricas`,x=`epFeed`,S=async()=>{let e=_();if(!e)return`<div class="epd_page"><div class="epd_empty"><i class="fas fa-lock"></i><p>Sin sesión activa.</p></div></div>`;let r=o(e.nombres||e.nombre||`Administrador`),i=s(e.nombres||e.nombre||``),l=e.foto||null,u=c(e.empresa||`Linkwii`),d=a(v)??`—`,f=a(y)??`—`,p=a(b)||{wpm:`—`,cert:`—`};return`
  <div class="epd_page">
    ...
    <div class="epd_hero">
      <div class="epd_hero_left">
        ...
        <div class="epd_hero_txt">
          ...
          <h1 class="epd_nombre">${r.split(` `)[0]}</h1>
          <div class="epd_tags">
      ...
      <div class="epd_hero_right">
        <div class="epd_date"><i class="fas fa-calendar-alt"></i> ${n()}</div>
      </div>
    </div>
    ...
    <div class="epd_feed_wrap">
  ...
  </div>`},C=async()=>{let t=_();t&&(e(document).off(`.epd`),await T(t),e(document).on(`click.epd`,`.nv_item`,function(t){t.preventDefault();let n=e(this).data(`page`);n&&l(async()=>{let{rutas:e}=await import(`./rutas-2bT34B5M.js`).then(e=>e.r);return{rutas:e}},[]).then(({rutas:e})=>e.navigate(`/${n}`))}),e(document).on(`click.epd`,`#epd_btn_sync`,async function(){let n=e(this).find(`i`).addClass(`fa-spin`);[v,y,b,x].forEach(e=>localStorage.removeItem(e)),await T(t,!0),setTimeout(()=>n.removeClass(`fa-spin`),500)}))},w=()=>{e(document).off(`.epd`)};async function T(e,t=!1){await Promise.all([E(e,t),D(e,t)])}async function E(t,n){if(!(!n&&a(v)!=null))try{let n=await u(p(m(g,`lecciones`),f(`empresa_id`,`==`,t.usuario)));n.empty&&(n=await u(p(m(g,`lecciones`),f(`gestor_id`,`==`,t.usuario))));let r=n.docs.map(e=>e.data()),a=r.length,o=r.map(e=>e.wpmMax||0),s=a>0?Math.round(o.reduce((e,t)=>e+t,0)/a):0,c=r.filter(e=>(e.completadas?.length||0)>=45&&(e.wpmMax||0)>=80).length,l=await u(p(m(g,`clases`),f(`empresa_id`,`==`,t.usuario)));l.empty&&(l=await u(p(m(g,`clases`),f(`gestor_id`,`==`,t.usuario))));let d=l.size;i(v,a,2),i(y,d,2),i(b,{wpm:s,cert:c},2),e(`#epd_k_emp`).text(a),e(`#epd_k_eqp`).text(d),e(`#epd_k_wpm`).text(s||`—`),e(`#epd_k_cert`).text(c)}catch(e){console.error(`[empresa] KPI Error`,e)}}async function D(e,t){if(!t){let e=a(x);if(e?.length){O(e);return}}try{let t=await u(p(m(g,`lecciones`),f(`empresa_id`,`==`,e.usuario),d(`ultPractica`,`desc`),h(10)));t.empty&&(t=await u(p(m(g,`lecciones`),f(`gestor_id`,`==`,e.usuario),d(`ultPractica`,`desc`),h(10))));let n=t.docs.map(e=>({usuario:e.id,...e.data()}));i(x,n,1/12),O(n)}catch{try{let t=await u(p(m(g,`lecciones`),f(`empresa_id`,`==`,e.usuario),h(10)));t.empty&&(t=await u(p(m(g,`lecciones`),f(`gestor_id`,`==`,e.usuario),h(10)))),O(t.docs.map(e=>({usuario:e.id,...e.data()})))}catch{O([])}}}function O(t){if(!t.length){e(`#epd_feed_body`).html(`
      <div class="epd_empty">
      ...
      <div class="epd_feed_item">
        ...
        <div class="epd_fi_time">${e.ultPractica?.toDate?r(e.ultPractica):`Reciente`}</div>
