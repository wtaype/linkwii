import{t as e}from"./vendor-CdV9uUFu.js";import{a as t,d as n,m as r,p as i,r as a,s as o,t as s,u as c}from"./widev-DUThRLxv.js";import"./wii-D6UWjlDJ.js";import{t as l}from"./preload-helper-BYCM-Flg.js";import{a as u,c as d,f,l as p,o as m,p as h}from"./firebase-D8eePon0.js";import{db as g}from"./firebase-Be0ozyG_.js";var _=()=>i(`wiSmile`),v=`epTotalEmpleados`,y=`epTotalEquipos`,b=`epMetricas`,x=`epFeed`,S=async()=>{let e=_();if(!e)return`<div class="epd_page"><div class="epd_empty"><i class="fas fa-lock"></i><p>Sin sesión activa.</p></div></div>`;let n=a(e.nombres||e.nombre||`Administrador`),r=o(e.nombres||e.nombre||``),l=e.foto||null,u=s(e.empresa||`Linkwii`),d=i(v)??`—`,f=i(y)??`—`,p=i(b)||{wpm:`—`,cert:`—`};return`
  ...
  </div>`},C=async()=>{let t=_();t&&(e(document).off(`.epd`),await T(t),e(document).on(`click.epd`,`.nv_item`,function(t){t.preventDefault();let n=e(this).data(`page`);n&&l(async()=>{let{rutas:e}=await import(`./rutas-BAajwB0_.js`).then(e=>e.r);return{rutas:e}},[]).then(({rutas:e})=>e.navigate(`/${n}`))}),e(document).on(`click.epd`,`#epd_btn_sync`,async function(){let n=e(this).find(`i`).addClass(`fa-spin`);[v,y,b,x].forEach(e=>localStorage.removeItem(e)),await T(t,!0),setTimeout(()=>n.removeClass(`fa-spin`),500)}))},w=()=>{e(document).off(`.epd`)};async function T(e,t=!1){await Promise.all([E(e,t),D(e,t)])}async function E(t,n){if(!(!n&&i(v)!=null))try{let n=await u(p(h(g,`lecciones`),f(`empresa_id`,`==`,t.usuario)));n.empty&&(n=await u(p(h(g,`lecciones`),f(`gestor_id`,`==`,t.usuario))));let i=n.docs.map(e=>e.data()),a=i.length,o=i.map(e=>e.wpmMax||0),s=a>0?Math.round(o.reduce((e,t)=>e+t,0)/a):0,c=i.filter(e=>(e.completadas?.length||0)>=45&&(e.wpmMax||0)>=80).length,l=await u(p(h(g,`clases`),f(`empresa_id`,`==`,t.usuario)));l.empty&&(l=await u(p(h(g,`clases`),f(`gestor_id`,`==`,t.usuario))));let d=l.size;r(v,a,2),r(y,d,2),r(b,{wpm:s,cert:c},2),e(`#epd_k_emp`).text(a),e(`#epd_k_eqp`).text(d),e(`#epd_k_wpm`).text(s||`—`),e(`#epd_k_cert`).text(c)}catch(e){console.error(`[empresa] KPI Error`,e)}}async function D(e,t){if(!t){let e=i(x);if(e?.length){O(e);return}}try{let t=await u(p(h(g,`lecciones`),f(`empresa_id`,`==`,e.usuario),d(`ultPractica`,`desc`),m(10)));t.empty&&(t=await u(p(h(g,`lecciones`),f(`gestor_id`,`==`,e.usuario),d(`ultPractica`,`desc`),m(10))));let n=t.docs.map(e=>({usuario:e.id,...e.data()}));r(x,n,1/12),O(n)}catch{try{let t=await u(p(h(g,`lecciones`),f(`empresa_id`,`==`,e.usuario),m(10)));t.empty&&(t=await u(p(h(g,`lecciones`),f(`gestor_id`,`==`,e.usuario),m(10)))),O(t.docs.map(e=>({usuario:e.id,...e.data()})))}catch{O([])}}}function O(t){if(!t.length){e(`#epd_feed_body`).html(`
      <div class="epd_empty">
